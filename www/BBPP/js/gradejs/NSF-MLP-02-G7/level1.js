Game.NSF_MLP_02_G7level1 = function () { };


Game.NSF_MLP_02_G7level1.prototype =
{
    init: function (param, score) {
        ...
        _this.Ask_Question1 = _this.createAudio("NSF_MLP_02_G7_a1");
        _this.Ask_Question2 = _this.createAudio("NSF_MLP_02_G7_a2");

        //edited for baseurl online apk
        telInitializerbbpp.gameIdInit("NSF_MLP_02_G7", gradeSelected);// first Tele call
    ...
    createAudio: function (src) {
        ...
        audiosrc.setAttribute("src", window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/" + src + ".mp3");
    ...
    Initial_randomizing: function () {
        ...
        // ●	one term is proper fraction and another is improper
        // ●	a/b * c/d ;
        // ○	b and d  always <=10
        // ○	  ‘b’ and ‘c’ such that b * c <= 30
        // ○	If a<b then c >d
        // ●	Some examples :
        // ○	⅔ * 7/6 = 14/18
        // ○	⅘  * 6/5 = 24/25
        ...
        _this.denominatorB = 2; //generate 2 to 10
        _this.numeratorA = 1;//generate 1 to 9

        _this.numeratorC = 13;// generate 3 to 14// or else we can make it as 13 max and 3 min. then we will get 15 to 3 

        _this.denominatorD = 4;// generate 2 to 13


        for (let i = 0; i < 6; i++) {

            _this.denominatorB = Math.floor(Math.random() * (9) + 2); //generate 2 to 10
            _this.numeratorA = Math.floor(Math.random() * (_this.denominatorB - 1) + 1);//generate 1 to 9

            _this.numeratorC = Math.floor(Math.random() * (15 - 3) + 3);// generate 3 to 14// or else we can make it as 13 max and 3 min. then we will get 15 to 3 


            if (_this.numeratorC <= 10) {
                _this.denominatorD = Math.floor(Math.random() * (_this.numeratorC - 2) + 2);// generate 2 to 13
            }
            else {
                _this.denominatorD = Math.floor(Math.random() * (10 - 2) + 2);// generate 2 to 9

            }

            while ((_this.numeratorA * _this.numeratorC > 30) || (_this.denominatorB * _this.denominatorD > 30) || (_this.denominatorB * _this.numeratorC > 30)) {

                _this.denominatorB = Math.floor(Math.random() * (9) + 2); //generate 2 to 10
                _this.numeratorA = Math.floor(Math.random() * (_this.denominatorB - 1) + 1);

                _this.numeratorC = Math.floor(Math.random() * (15 - 3) + 3);


                if (_this.numeratorC <= 10) {
                    _this.denominatorD = Math.floor(Math.random() * (_this.numeratorC - 2) + 2);
                }
                else {
                    _this.denominatorD = Math.floor(Math.random() * (10 - 2) + 2);
        ...
        for (var i = 0; i < 6; i++) {
            ...
            _this.bc.push(_this.bArr[i] * _this.cArr[i])
        }
    ...
    eraserDrop: function (target) {
        ...
        if (_this.finamnum == true) {
            ...
            if (_this.finalAnsGrpNum.length > _this.ans3Grp.length) {
                ...
                if (_this.finalAnsGrpNum.length == 0 && _this.finalBorder.length > 0) {
                    console.log("final border > 0.......")
                    _this.finalBorder.destroy();
                    _this.finalBorder.visible = false;
                }
    ...
    remakeCubes: function (grp) {
        ...
        if (_this.ans3Grp.length > 0 && (_this.ans3Grp.length > 10 || _this.ans3Grp.length > (_this.ans3Grp.getChildAt(0).frame + 1))) {
            ...
            _this.makeRectBox(_this.ans3Grp.getChildAt(q).frame + 1, 221 - 20, borderboxc);//322 - 20
            _this.borderBox3.addChild(_this.graphics);
            if (_this.ans3GrpFinal.length == 0 && _this.borderBox3.length > 0) {
                _this.borderBox3.destroy();
                _this.borderBox3.visible = false;
            }
    ...
    makeupperRodsGGrp: function (x, y, z, t) {
        ...
        if (_this.ans3GrpFinal.length > 10 || _this.ans3GrpFinal.length > (_this.ans3GrpFinal.getChildAt(0).frame + 1)) {
            ...
            if (_this.ans3GrpFinal.length == 0 && _this.borderBox31.length > 0) {
                _this.borderBox31.destroy();
                _this.borderBox31.visible = false;
            }
        }
    ...
    dragUpdateNumRod: function (target) {
        ...
        if (_this.ans3Grp.length <= 10) {
        ...
        else if (_this.ans3Grp.length > 10) {
            console.log("else if.....")
            console.log("target", Number(target.name));
            k = Number(target.name) % (_this.ans3Grp.length * _this.bArr[_this.count1]);
            console.log("k = ", k)
            k2 = target.name;

            if (Number(target.name) > _this.ans3Grp.length && Number(target.name) < (_this.ans3Grp.length * _this.bArr[_this.count1])) {
                for (let i = target.name; i >= _this.ans3Grp.length; i--) {// i =20 
                ...
                for (let i = target.name + 1; i < (_this.ans3Grp.length * _this.bArr[_this.count1]); i++) {//grp.getChildAt(0).frame + 
            ...
            else if (Number(target.name) < _this.ans3Grp.length && Number(target.name) >= 0) {
                for (let i = target.name; i >= 0; i--) {
                    console.log("back pos")
                    grp.getChildAt(i).y = draggedCubey
                    grp.getChildAt(i).x = draggedCubex - 19 * backpos;
                    backpos++;
                ...
                for (let i = target.name + 1; i < _this.ans3Grp.length; i++) {//grp.getChildAt(0).frame + 
                    console.log("front pos")
                    grp.getChildAt(i).y = draggedCubey
                    grp.getChildAt(i).x = draggedCubex + 19 * frontpos;
                    frontpos++;
    ...
    dA6: function () {
      ...
      DemoVideo: function () {
        ...
        _this.demoAudio1src.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d1.mp3");
        _this.demoAudio1.appendChild(_this.demoAudio1src);

        //* Multiply   1/4  * 5/3 
        _this.demoAudio2 = document.createElement('audio');
        _this.demoAudio2src = document.createElement('source');
        _this.demoAudio2src.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d2.mp3");
        ...
        _this.demoAudio3src.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d3.mp3");
        ...
        _this.demoAudio4src.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d4.mp3");
        ...
        _this.demoAudio5src.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d5.mp3");
        ...
        _this.demoAudio6src.setAttribute("src", window.baseUrl +"questionSounds/NSF-MLP-02-G7/" + _this.languageSelected + "/NSF_MLP_02_G7_d6.mp3");
        ...
        _this.q1Soundsrc.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" +
            _this.languageSelected + "/NSF_MLP_02_G7_a1.mp3");
        _this.q1Sound.appendChild(_this.q1Soundsrc);

        //* Enter the answer
        _this.q2Sound = document.createElement('audio');
        _this.q2Soundsrc = document.createElement('source');
        _this.q2Soundsrc.setAttribute("src",window.baseUrl + "questionSounds/NSF-MLP-02-G7/" +
            _this.languageSelected + "/NSF_MLP_02_G7_a2.mp3");
